...
public class ArrayOp2D {
    ...
    private static void verifyLength(double[][] array1, double[][] array2) throws ArrayOpException{
        if (array1.length != array2.length || array1[0].length != array2[0].length) {
            throw new ArrayOp2DException(DimensionsError(array1, array2));
        }
    }
    ...
    public static double[][] sum(double[][] array1, double[][] array2) throws ArrayOpException{
        ...
        for (int i = 0; i < l; i++) {
            result[i] = ArrayOp1D.sum(array1[i], array2[i]);
        }
        return result;
    }
    ...
    public static double[][] divide(double[][] array1, double[][] array2) throws ArrayOpException {
        ...
        boolean zeroFree;
        int l = array1.length;
        for (double[] raw: array2) {
            // verify whether the i-th row has a zero value
            zeroFree = Arrays.stream(raw).anyMatch(x -> x == 0);
            if (zeroFree) {
                throw new ArrayOp2DException(ZeroDivError(raw));
            }
        }
